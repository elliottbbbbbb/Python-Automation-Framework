...
from osrsbot.services.arduino_mouse_service import SERIAL_AVAILABLE, ArduinoMouseService
...
class TestArduinoMouseService:
    ...
    def mock_serial(self, monkeypatch):
        ...
        if SERIAL_AVAILABLE:
            ...
            monkeypatch.setattr(
                "osrsbot.services.arduino_mouse_service.SERIAL_AVAILABLE", True
            )
            monkeypatch.setattr(
    ...
    def test_initialization_with_available_arduino(self, mouse_config, mock_serial):
        ...
        service = ArduinoMouseService(
            config=mouse_config, serial_port="COM3", fallback_to_software=False
        )
    ...
    def test_initialization_pyserial_not_installed(self, mouse_config, monkeypatch):
        ...
        monkeypatch.setattr(
            "osrsbot.services.arduino_mouse_service.SERIAL_AVAILABLE", False
        )

        # Should fall back to software mouse
        service = ArduinoMouseService(config=mouse_config, fallback_to_software=True)

        assert service._fallback is not None
        assert service._connection is None

    def test_initialization_arduino_unavailable_with_fallback(
        self, mouse_config, mock_serial
    ):
        ...
        with patch("serial.Serial", mock_serial_class.Serial):
            service = ArduinoMouseService(
                config=mouse_config, serial_port="COM999", fallback_to_software=True
            )

        assert service._fallback is not None
        assert service._connection is None

    def test_initialization_arduino_unavailable_no_fallback(
        self, mouse_config, mock_serial
    ):
    ...
    def test_move_to_uses_fallback(self, mouse_config, mock_serial):
        ...
        with patch("serial.Serial", mock_serial_class.Serial):
            service = ArduinoMouseService(
                config=mouse_config, fallback_to_software=True
            )
    ...
    def test_position_correction_retry(self, mouse_config, mock_serial):
        ...
        with patch.object(
            service, "_get_cursor_position", side_effect=lambda: next(position_iter)
        ):
