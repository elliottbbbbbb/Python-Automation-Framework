...
class InventoryActions:
    ...
    def click_slot(
        ...
        logger.debug(f"Clicking slot {slot_num} at ({rel_x}, {rel_y}) -> ({abs_x}, {abs_y})")
    ...
    def ensure_open(self) -> bool:
        ...
        button_pos = self.coords.resolve_ui_button("inventory_button", force_detect=True)
        if button_pos:
            rel_x, rel_y = button_pos
            abs_x, abs_y = self.coords.to_absolute(rel_x, rel_y)
    ...
    def drop_all_except(
        self,
        keep_slots: Optional[list[int]] = None
    ) -> int:
        """
