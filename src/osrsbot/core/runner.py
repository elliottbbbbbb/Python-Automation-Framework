    ...
    def __init__(self, window_title: str, config_file: str = "config.json") -> None:
        ...
        try:
            if not getattr(sys, "frozen", False):
                # Dev mode: always use the standard src/osrsbot/config.json location
                logger.debug("Dev mode: using default config location")
                self.config = Config()
            else:
                # Exe mode: look next to the exe first, then fall back to bundled
                ...
                try:
                    ...
                    logger.warning(f"Primary config not found, trying bundled: {bundled_config}")
