...
States:
    ...
    Navigate to vorkath from house portal. (I think this should include
        before and after we travel to vorkaths island.)
    COMBAT - This is going to be a bit more complex, the combat phase is
        going to have subphases which can be triggered depending on what
        phase of the boss we are in.
    Do we want to implement the phases sequentially? I don't remember if
        vorkath is similar to zulrah in the sense that the possible phases
        you can go through depend on external factors. If so, their might
        need to be some sort of subphase for combat as in combat(dodging
        the 1 shot ability, dodging the green poison on the floor)
    I would also need to figure out whether or not we would allow multiple
        states to exists at once. For example if we need to dodge green
        poison and then idk attack - `probably not tho thinking about it.
...
class VorkathStates(Enum):
    ...
    IDLE = "IDLE"
    NAVIGATE_TO_VORKATH = (
        "navigate_to_vorkath"  # This is going to include tp and walking
    )
    COMBAT = "combat"
...
class VorkathStateMachineBot(StateMachineBot):
    ...
    def __init__(self, *args, debug_ui=False, **kwargs):
        super().__init__(
            *args, debug_ui=debug_ui, **kwargs, script_name="Vorkath (State Machine)"
        )
        ...
        return build_metadata_dict(
            VorkathStates,
            {
                ...
                VorkathStates.NAVIGATE_TO_VORKATH: {
                    ...
                    "description": (
                        "Navigate to vorkath from house portal "
                        "(includes teleporting to house)"
                    ),
                    "max_retries": 3,
                    "timeout": 30.0,
