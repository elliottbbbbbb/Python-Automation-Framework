...
class HerbloreBot(BankstanderBot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if config:
            manual = config.get("herblore_starting_level", default="")
            if manual:
                try:
                ...
                wom = self._initial_wom_lookup(config, "herblore")
                if wom is not None:
                    starting_level = wom

        tier = get_best_tier(starting_level)
        tool_name = tier["tool_name"] if tier else "marrentill potion (unf)"
        material_name = tier["material_name"] if tier else "unicorn horn dust"
        product_name = tier["product_name"] if tier else "antipoison"

        logger.info(f"Herblore level {starting_level} -> crafting {product_name}")

        # Load templates (tool changes per tier for herblore)
        tool_template = load_item_templates(images_dir, tool_name)
        ...
        self._menu_templates = (
            glob_templates(images_dir, "ui_templates/herblore_menu*.png")
            or [str(images_dir / "ui_templates" / "herblore_menu.png")]
        )
        ...
        self._levelup_templates = (
            glob_templates(images_dir, "ui_templates/herblore_level_up*.png")
            or [str(images_dir / "ui_templates" / "herblore_level_up.png")]
        )
    ...
    def _reload_templates(self, tier: dict) -> None:
        """Reload tool + material + product templates after tier switch."""
        old = self._product_name
        self._tool_name = tier["tool_name"]
        self._material_name = tier["material_name"]
        self._product_name = tier["product_name"]
        # Herblore: tool (unf potion) changes per tier
        self._tool_template = load_item_templates(self._images_dir, tier["tool_name"])
        ...
        logger.info(f"TIER: Switched from {old} to {tier['product_name']}")
    ...
    def _wom_level_check(self) -> None:
        ...
        if self._cycles % WOM_CHECK_INTERVAL != 0 or self._cycles == 0:
            return
        wom_level = self.state.get_skill_level_from_wom("herblore", force_update=True)
        ...
        try:
            ...
            if not actions.click_template(self._tool_template, self._tool_name, threshold=0.8):
                ...
                fallback = find_best_available_tier(
                    self, HERBLORE_TIERS, self._images_dir,
                    self._current_level, self._material_name,
                    exclude_current=True, tool_is_shared=False,
                )
                if fallback:
            ...
            # Withdraw material (secondary ingredient)
    ...
    def process_items(self, context: StateExecutionContext) -> StateResult:
        try:
            ...
            # Click tool (unf potion) in inventory
            ...
            # Click material (secondary) in inventory
